...
public static class ExtendedCollisionBinding
{
    ...
    {
        return t.AddBinding(OnComponentCollision2DWith<T>(t.gameObject, eventType).Subscribe(c=> collision(c.gameObject.GetComponent<T>(), c )));
    }

    public static IObservable<T> OnComponentCollisionWith<T>(this GameObject t, CollisionEventType eventType) where T : MonoBehaviour
    {
        return t.OnCollisionObservable(eventType).Where(p => p.gameObject.GetComponent<T>() != null).Select(p => p.gameObject.GetComponent<T>());
    }
    public static IObservable<Collision2D> OnComponentCollision2DWith<T>(this GameObject t, CollisionEventType eventType) where T : MonoBehaviour
    {
        return t.OnCollision2DObservable(eventType).Where(p => p.gameObject.GetComponent<T>() != null).Select(p => p);
    }
